...
import type { ScoringConfig, SourceInterpretationKonfigurasi } from '../../types';
import { Button, Card, Select } from '../ui';
import { downloadFile } from '../../utils/export';
import { clearAdminDataKey, saveAuxKonfigurasi } from '../../utils/adminStorage';
import { validateHubertusConfig, validateRusdiKonfigurasi } from '../../utils/sourceInterpretations';
...
const template = (sourceName: 'Rusdi Maslim' | 'Hubertus'): SourceInterpretationKonfigurasi => ({
  sourceName,
...
const SourcePanel = ({ source, config, codeTypeConfig, scoringConfig, onRefresh, toast }: { source: SourceKey; config: unknown; codeTypeConfig: unknown; scoringConfig: ScoringKonfigurasi | null; onRefresh: () => void; toast: (message: string, tone?: 'teal' | 'amber' | 'rose') => void }) => {
  ...
  const scaleKeys = Object.keys(((active as SourceInterpretationKonfigurasi | null)?.scaleInterpretations ?? {}) as Record<string, unknown>);
  const codeTypeKeys = Object.keys(((codeDraft ?? codeTypeKonfigurasi ?? (active as SourceInterpretationKonfigurasi | null)?.codeTypeInterpretations ?? {}) as Record<string, unknown>));
  const readFile = async (file: File | undefined, setter: (value: unknown) => void) => { if (!file) return; setter(JSON.parse(await file.text())); };
  const previewValue = scalePreview ? ((active as SourceInterpretationKonfigurasi | null)?.scaleInterpretations ?? {})[scalePreview] : null;
  return <Card>
    <PanelTitle title={`Impor interpretasi ${info.label}`} subtitle={`Unggah interpretationKonfigurasi dan codeTypeKonfigurasi ${info.label}. Nama sumber wajib “${info.label}”; konten manual tidak disertakan di kode sumber.`} />
    <div className="mt-4 grid gap-3 lg:grid-cols-2">
      <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
        <h3 className="font-black">Impor interpretationKonfigurasi {info.label}</h3>
