...
const SourcePanel = ({ source, config, codeTypeConfig, scoringConfig, onRefresh, toast }: { source: SourceKey; config: unknown; codeTypeConfig: unknown; scoringConfig: ScoringConfig | null; onRefresh: () => void; toast: (message: string, tone?: 'teal' | 'amber' | 'rose') => void }) => {
  ...
  return <Card>
    <PanelTitle title={`Import Interpretasi ${info.label}`} subtitle={`Upload interpretationConfig dan codeTypeConfig ${info.label}. Source name wajib “${info.label}”; konten manual tidak disertakan di source code.`} />
    <div className="mt-4 grid gap-3 lg:grid-cols-2">
      <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
        <h3 className="font-black">Import interpretationConfig {info.label}</h3>
        <div className="mt-3 flex flex-wrap gap-3"><input className="block w-full text-sm sm:w-auto" type="file" accept=".json" onChange={(event) => readFile(event.target.files?.[0], setDraft).then(() => toast(`Config ${info.label} terbaca.`, 'teal')).catch((error) => toast(error instanceof Error ? error.message : 'Gagal import config.', 'rose'))} /><TemplateButton filename={`template_interpretasi_${source}.json`} data={info.template} /><Button variant="ghost" onClick={() => toast(validation.valid ? 'Validasi config berhasil.' : 'Validasi menemukan masalah.', validation.valid ? 'teal' : 'amber')}>Validasi</Button></div>
        <div className="mt-4"><ValidationMessages result={validation} /></div>
        <div className="mt-4 flex flex-wrap gap-3"><Button disabled={!draft || !validation.valid} onClick={() => { saveAuxConfig(info.interpretationKey, draft); setDraft(null); onRefresh(); toast(`Interpretasi ${info.label} disimpan.`, 'teal'); }}>Simpan</Button><Button variant="secondary" disabled={!active} onClick={() => downloadFile(`interpretasi-${source}.json`, JSON.stringify(active, null, 2))}>Export</Button><Button variant="danger" onClick={() => { clearAdminDataKey(info.interpretationKey); onRefresh(); toast(`Interpretasi ${info.label} dihapus.`, 'amber'); }}>Hapus</Button></div>
      </div>
      <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
        <h3 className="font-black">Import codeTypeConfig {info.label}</h3>
        <div className="mt-3 flex flex-wrap gap-3"><input className="block w-full text-sm sm:w-auto" type="file" accept=".json" onChange={(event) => readFile(event.target.files?.[0], setCodeDraft).then(() => toast(`Code type ${info.label} terbaca.`, 'teal')).catch((error) => toast(error instanceof Error ? error.message : 'Gagal import code type.', 'rose'))} /><Button disabled={!codeDraft} onClick={() => { saveAuxConfig(info.codeTypeKey, codeDraft); setCodeDraft(null); onRefresh(); toast(`Code type ${info.label} disimpan.`, 'teal'); }}>Simpan code type</Button><Button variant="secondary" disabled={!codeTypeConfig && !codeDraft} onClick={() => downloadFile(`code-type-${source}.json`, JSON.stringify(codeDraft ?? codeTypeConfig, null, 2))}>Export</Button><Button variant="danger" onClick={() => { clearAdminDataKey(info.codeTypeKey); onRefresh(); toast(`Code type ${info.label} dihapus.`, 'amber'); }}>Hapus</Button></div>
