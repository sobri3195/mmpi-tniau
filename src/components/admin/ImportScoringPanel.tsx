...
export const ImportScoringPanel = ({ questions, config, onRefresh, toast }: { questions: Question[]; config: ScoringConfig | null; onRefresh: () => void; toast: (message: string, tone?: 'teal' | 'amber' | 'rose') => void }) => {
  ...
  return <Card>
    <PanelTitle title="Impor konfigurasi scoring" subtitle="Unggah scoringConfig.json. Kunci scoring MMPI asli tidak boleh disimpan di kode sumber." status={<Badge tone={validation?.valid ? 'teal' : 'amber'}>{scales.length} skala</Badge>} />
    ...
    <div className="mt-5 flex flex-wrap gap-3"><Button disabled={!draft || !validation?.valid} onClick={() => { if (draft) saveAdminScoringConfig(draft); setDraft(null); onRefresh(); toast('Scoring config disimpan.', 'teal'); }}>Simpan konfigurasi scoring</Button><Button variant="danger" onClick={() => { clearAdminDataKey('scoringConfig'); onRefresh(); toast('Scoring config dihapus.', 'amber'); }}>Hapus konfigurasi scoring</Button><Button variant="secondary" disabled={!active} onClick={() => active && downloadFile('scoringConfig-export.json', JSON.stringify(active, null, 2))}>Ekspor konfigurasi scoring</Button></div>
  </Card>;
};
