...
export const SystemReadinessCheck = ({ questions, scoringConfig, normTable, interpretationConfig, codeTypeConfig, settings }: { questions: Question[]; scoringConfig: ScoringConfig | null; normTable: unknown; interpretationConfig: unknown; codeTypeConfig: unknown; settings: AdminReportSettings }) => {
  ...
  return <Card><h2 className="text-xl font-black">Status Kesiapan Sistem</h2><p className="mt-1 text-sm text-slate-500">Interpretasi spesialis aktif ketika seluruh komponen pendukung sudah lengkap.</p><div className="mt-5 grid gap-4">
    ...
    <StatusRow title="C. Siap Interpretasi Klinis" ready={clinical.ready} partial={scoringReady || Boolean(normTable || interpretationConfig)} notes={clinical.ready ? ['Bank soal, scoringConfig, norma T-score, dan interpretationConfig lengkap.', 'Konfigurasi sudah diverifikasi.', 'Skala validity dan clinical tersedia.'] : clinical.errors} />
