...
export const BackupRestorePanel = ({ onRefresh, toast }: { onRefresh: () => void; toast: (message: string, tone?: 'teal' | 'amber' | 'rose') => void }) => {
  ...
  const importBackup = async (file?: File) => {
    ...
    onRefresh();
    toast('Backup berhasil diimport.', 'teal');
  };
  return <Card><PanelTitle title="Reset dan Backup" subtitle="Export/import seluruh data aplikasi dan reset data tertentu. Reset wajib dikonfirmasi dengan mengetik RESET." />
    <AlertBox tone="amber">Sebelum reset, sistem meminta konfirmasi: <strong>Ketik RESET untuk melanjutkan.</strong></AlertBox>
    <div className="mt-5 grid gap-3 sm:flex sm:flex-wrap"><Button onClick={() => downloadFile(`backup-sppg-mmpi2-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify(createBackupPayload(), null, 2))}>Export Backup JSON</Button><input className="block w-full text-sm sm:w-auto" type="file" accept=".json" onChange={(event) => importBackup(event.target.files?.[0]).catch((error) => toast(error instanceof Error ? error.message : 'Gagal import backup.', 'rose'))} /></div>
