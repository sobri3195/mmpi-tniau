...
export const SummaryAnalysisSection = ({ result, config, rhForm }: { result: AssessmentResult; config?: SummaryAnalysisConfig | null; rhForm?: RHForm | null }) => {
  ...
  return <Card className="summary-analysis-report"><div className="summary-screen-header flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-bold uppercase tracking-wide text-teal-700">Analisa Ringkas TNI AU</p><h2 className="text-2xl font-black">LAPORAN HASIL TES MMPI-2</h2><p className="mt-1 text-sm text-slate-500">Ringkasan bantu untuk pemeriksa; bukan diagnosis final otomatis.</p></div><div className="flex flex-wrap gap-2"><Badge tone={analysis.isAutoDefault || analysis.isDemo ? 'amber' : 'teal'}>{analysis.badgeLabel ?? (analysis.isDemo ? 'Demo' : 'Admin imported')}</Badge></div></div>
    ...
    {analysis.isAutoDefault && <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-900">{analysis.disclaimer}</div>}
