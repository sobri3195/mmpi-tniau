...
export const FinalReportSignature = ({ review }: { review: SpecialistReview }) => (
  <Card className="border-teal-200 bg-teal-50/60 dark:border-teal-900 dark:bg-teal-950/30">
    ...
    <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><p><strong>Reviewer:</strong> {review.reviewerName || '-'}</p><p><strong>Jabatan:</strong> {review.reviewerTitle || '-'}</p><p><strong>Nomor izin:</strong> {review.licenseNumber || '-'}</p><p><strong>Direview:</strong> {review.reviewedAt ? new Date(review.reviewedAt).toLocaleString('id-ID') : '-'}</p></div>
  </Card>
);
