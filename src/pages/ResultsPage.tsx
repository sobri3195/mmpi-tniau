  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 sm:py-8">
      <Card className="mb-6 report-header">
        <div className="flex flex-wrap justify-between gap-4"><div><p className="text-sm font-bold text-teal-600">TNI AU / SPPG</p><h1 className="text-2xl font-black sm:text-3xl">Laporan Hasil Asesmen MMPI TNI AU/SPPG</h1><p className="text-slate-500">Header TNI AU/SPPG — asesmen satu kali, interpretasi ditampilkan di halaman hasil.</p></div><div className="flex flex-col items-start gap-2 sm:items-end"><Badge tone={result.status === 'Perlu Review' ? 'amber' : 'teal'}>{result.status}</Badge><Badge tone={validityTone}>{result.validityStatus?.label ?? 'Validitas belum dinilai'}</Badge></div></div>
      ...
      <section className={tab === 'summary' ? 'space-y-6' : 'hidden print:block print:space-y-6'}>
        {!report.isDemo && <Card><h2 className="mb-3 text-xl font-black">Ringkasan Skor Utama</h2><p className="text-sm leading-6">{report.executiveSummary}</p><div className="mt-3"><Badge tone={report.reviewStatus === 'Dapat ditelaah' ? 'teal' : report.reviewStatus === 'Perlu review/retest' ? 'rose' : 'amber'}>{report.reviewStatus}</Badge></div></Card>}
...
const SourceInterpretationSection = ({ title, interpretation, unavailable }: { title: string; interpretation?: SourceInterpretationResult; unavailable: string }) => {
  ...
  return <div><h2 className="text-xl font-black">{title}</h2><div className="mt-4 grid gap-4"><Narrative title="Validitas" text={interpretation.validityNarrative} /><Narrative title="Klinis / Skala" text={interpretation.clinicalNarrative} /><Narrative title="Code Type" text={interpretation.codeTypeNarrative} /><Narrative title="Domain" text={interpretation.domainNarrative} />{interpretation.recommendations.length ? <div><h3 className="font-black">Rekomendasi dari config</h3><ul className="mt-2 list-disc pl-5 text-sm leading-6">{interpretation.recommendations.map((item) => <li key={item}>{item}</li>)}</ul></div> : <p className="text-sm text-slate-500">Rekomendasi belum tersedia dalam config.</p>}</div></div>;
};

const Narrative = ({ title, text }: { title: string; text?: string }) => <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800"><h3 className="font-black">{title}</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{text || 'Belum tersedia dalam config yang diimpor admin.'}</p></div>;
...
const SpecialistFinalization = ({ result }: { result: AssessmentResult }) => <div><h2 className="text-xl font-black">Catatan Spesialis / Finalisasi</h2><p className="mt-2 text-sm text-slate-500">Keputusan final tetap dibuat oleh spesialis/dokter jiwa/psikolog klinis. Pilihan di bawah adalah bahan finalisasi, bukan diagnosis otomatis.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><label>Mode final<Select defaultValue={result.specialistReview?.selectedFinalInterpretation ?? 'not_selected'}><option value="rusdi_maslim">Final mengikuti Rusdi Maslim</option><option value="hubertus">Final mengikuti Hubertus</option><option value="combined_professional_review">Final berdasarkan telaah profesional gabungan</option><option value="further_interview">Perlu wawancara lanjutan</option><option value="retest_required">Perlu retest</option><option value="referral_required">Perlu rujukan</option><option value="not_selected">Belum dipilih</option></Select></label><label>Reviewer<Textarea readOnly value={`${result.specialistReview?.reviewerName || ''}${result.specialistReview?.reviewerTitle ? `, ${result.specialistReview.reviewerTitle}` : ''}`} placeholder="Diisi melalui halaman Specialist Review" /></label></div><div className="mt-4 grid gap-4"><Narrative title="Catatan klinis" text={result.specialistReview?.clinicalImpression || result.specialistReview?.validityNotes} /><Narrative title="Kesimpulan final spesialis" text={result.specialistReview?.finalConclusion} /><Narrative title="Rekomendasi akhir setelah review profesional" text={result.specialistReview?.recommendations} /></div></div>;

const List = ({ title, items }: { title: string; items?: string[] }) => <div><h3 className="font-black">{title}</h3>{items?.length ? <ul className="mt-2 list-disc pl-5">{items.map((item) => <li key={item}>{item}</li>)}</ul> : <p className="mt-2 text-slate-500">Belum tersedia karena salah satu/kedua config belum tersedia atau belum valid.</p>}</div>;
