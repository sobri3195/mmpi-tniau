...
export const TokenAccessPage = ({ reason, onVerified }: { reason?: string; onVerified: () => void }) => {
  ...
  return <div className="mx-auto max-w-xl px-4 py-10">
    <Card>
      ...
      <p className="mt-2 text-slate-600 dark:text-slate-300">Masukkan token akses dan kunci unik peserta yang diberikan admin. Token hanya disimpan di perangkat ini dan tidak dimasukkan ke URL.</p>
      <div className="mt-6 grid gap-4">
        <label>Token akses<Input autoFocus value={token} placeholder="Contoh: TNI-AU-AB12-CD34" onChange={(e) => setToken(e.target.value.toUpperCase())} /></label>
        <label>Kunci unik<Input value={uniqueKey} placeholder="Contoh: PESERTA-2026-0001" onChange={(e) => setUniqueKey(e.target.value.toUpperCase())} /></label>
      </div>
