...
export const TestPage = ({ session, questions, onSubmit, onExit, onChange }: { session: CurrentSession; questions: Question[]; onSubmit: (s: CurrentSession) => void; onExit: () => void; onChange: (s: CurrentSession) => void }) => {
  ...
  const renderQuestion = (q: Question, index: number) => (
    <div key={q.id} className="rounded-3xl border border-slate-200 p-5 dark:border-slate-800">
      <div className="flex items-center justify-between gap-3"><Badge>Soal {index + 1} / {questions.length}</Badge><span className="font-mono text-xs text-slate-500">{q.code}</span></div>
      <p className="mt-4 text-lg font-semibold">{q.text}</p>
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {q.options.map((opt) => <button key={String(opt.value)} onClick={() => answer(q.id, opt.value)} className={`rounded-2xl border p-4 text-left font-bold transition ${local.answers[String(q.id)] === opt.value ? 'border-teal-500 bg-teal-50 text-teal-800 ring-4 ring-teal-100 dark:bg-teal-950 dark:text-teal-100 dark:ring-teal-900' : 'border-slate-200 hover:border-teal-300 dark:border-slate-700'}`}>{opt.label}</button>)}
      </div>
    </div>
  );
  if (!questions.length) return <div className="mx-auto max-w-3xl px-4 py-10"><Card><h1 className="text-2xl font-black">Bank soal belum tersedia</h1><p className="mt-2">Admin harus mengimport bank soal JSON/CSV resmi/berizin atau memuat dummy demo untuk pengujian.</p><Button className="mt-4" onClick={onExit}>Kembali</Button></Card></div>;
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <Card className="mb-6 no-print">
        ...
        <div className="mt-4 flex flex-wrap gap-2">{questions.map((q, i) => <button key={q.id} onClick={() => update({ currentIndex: i })} className={`h-9 w-9 rounded-xl text-sm font-bold ${local.answers[String(q.id)] !== undefined ? 'bg-teal-600 text-white' : 'bg-slate-100 dark:bg-slate-800'} ${i === local.currentIndex ? 'ring-2 ring-blue-500' : ''}`}>{i + 1}</button>)}</div>
        <div className="mt-4 flex flex-wrap gap-3"><Button variant="ghost" onClick={() => update({ mode: local.mode === 'single' ? 'list' : 'single' })}>Mode: {local.mode === 'single' ? 'Satu soal' : 'Daftar'}</Button><Button variant="ghost" onClick={onExit}>Simpan & Lanjutkan Nanti</Button></div>
      </Card>
      <Card>
        {local.mode === 'single' ? renderQuestion(question, local.currentIndex) : <div className="space-y-4">{questions.map(renderQuestion)}</div>}
        <div className="mt-6 flex flex-wrap justify-between gap-3 no-print">
          ...
          <div className="flex gap-3"><Button variant="ghost" disabled={local.currentIndex === questions.length - 1} onClick={() => update({ currentIndex: Math.min(questions.length - 1, local.currentIndex + 1) })}>Berikutnya</Button><Button disabled={missing.length > 0} onClick={() => onSubmit(local)}>Submit Hasil</Button></div>
        </div>
