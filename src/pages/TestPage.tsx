    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:py-8">
      ...
      <Card>
        ...
        <div className="mt-6 grid gap-3 no-print sm:flex sm:flex-wrap sm:justify-between">
          ...
          <div className="grid gap-3 sm:flex"><Button variant="ghost" disabled={local.currentIndex === questions.length - 1} onClick={() => update({ currentIndex: Math.min(questions.length - 1, local.currentIndex + 1) })}>Berikutnya</Button><Button disabled={missing.length > 0} onClick={() => onSubmit(local)}>Submit Hasil</Button></div>
        </div>
