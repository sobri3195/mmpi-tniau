...
import { clearScoringConfig, loadDemoQuestions, loadDemoScoringConfig, resetAllLocalData, resetParticipantData, saveQuestions, saveResults, saveScoringConfig } from '../utils/storage';
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2"><h2 className="font-black">Scoring Config</h2><p className="mt-2 text-sm text-slate-500">{(pendingConfig ?? config)?.instrumentName ?? 'Belum tersedia'}</p>{pendingConfig && <Badge tone="amber">Preview belum aktif</Badge>}<input className="mt-4 block w-full text-sm" type="file" accept=".json" onChange={(e) => importFile('scoring')(e.target.files?.[0])} />
          ...
          <div className={`mt-4 rounded-2xl p-4 text-sm font-semibold ${scoringSummary.isValid ? 'bg-teal-50 text-teal-800 dark:bg-teal-950 dark:text-teal-100' : 'bg-amber-50 text-amber-900 dark:bg-amber-950 dark:text-amber-100'}`}>Status: {scoringSummary.isValid ? 'valid' : scoringSummary.validationMessage}</div>
